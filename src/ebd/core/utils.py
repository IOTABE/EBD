...
from .models import Aluno, Classe
...
def read_xlsx_rows_from_file(file_obj) -> List[Tuple[str, Optional[datetime.date], str, str]]:
...
def read_csv_rows_from_file(file_obj) -> List[Tuple[str, Optional[datetime.date], str, str]]:
    ...
    for row in reader:
        ...
        if not nome or nome.lower() in ('nome', 'carimbo de data/hora'):
            continue
...
def process_alunos_import(alunos_data: List[Tuple[str, Optional[datetime.date], str, str]]) -> Tuple[int, int]:
    """Processa e salva uma lista de alunos no banco de dados."""
    criados = 0
    atualizados = 0
    for nome, nasc, tel, nome_classe in alunos_data:
